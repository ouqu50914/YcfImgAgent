    ...
    <div class="upscale-node">
        ...
        <div class="node-content">
            ...
            <div class="params-section">
                ...
                <!-- 放大倍数 -->
                <div class="param-item">
                    <div class="param-label">放大倍数</div>
                    <el-select v-model="scale" placeholder="选择放大倍数" size="small" class="param-select">
                        <el-option label="2倍 (2x)" :value="2" />
                        <el-option label="4倍 (4x)" :value="4" />
                    </el-select>
                </div>
            </div>
...
import { ZoomIn, CircleCheck } from '@element-plus/icons-vue';
import { upscaleImage } from '../../api/image';
...
const executeButtonText = computed(() => {
    ...
    return `开始放大 (消耗 ${executeCost.value} 积分)`;
});
...
const handleUpscale = async () => {
    ...
    try {
        ...
        console.log('[前端 UpscaleNode] 调用 upscaleImage，参数:', params);

        const res: any = await upscaleImage(params);
...
.upscale-node :deep(.el-input__inner),
.upscale-node :deep(.el-select__placeholder) {
    color: #b0b0b0;
}

.upscale-node :deep(.el-input__inner::placeholder) {
    color: #808080;
}
...
.upscale-node :deep(.vue-flow__handle) {
    opacity: 0;
    pointer-events: none;
    transition: opacity 0.15s ease;
}

.upscale-node:hover :deep(.vue-flow__handle) {
    opacity: 1;
    pointer-events: auto;
}
